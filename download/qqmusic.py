...
download_format_url = 'http://dl.stream.qqmusic.qq.com/{}.m4a?vkey={}&guid=8208467632&uin=0&fromtag=66'
listen_url='http://ws.stream.qqmusic.qq.com/C100{}.m4a?fromtag=0&guid=126548448'
songname="动物世界"
...
for i in range(len(media_mids)):
    fcg_res = requests.get(fcg_url.format(songmids[i], media_mids[i]), headers=headers)
    vkey = re.findall('"vkey":"(.*?)"', fcg_res.text)[0]
    download_names.append(temp_names[i].replace("\\", "").replace("/", "").replace(" ", "").replace('.', ''))
    download_urls.append(download_format_url.format(media_mids[i], vkey))
for i in range(len(songmids)):
    print(temp_names[i]+":"+listen_url.format(songmids[i]))
